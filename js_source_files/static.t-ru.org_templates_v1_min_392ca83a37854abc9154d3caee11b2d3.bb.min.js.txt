...
BB.rand_num = Math.round(Math.random() * 1e9);

BB.referrer = document.referrer || 'unknown';
...
$.browser = function(t) {
 var e = {};
 var n = /(chrome)[ \/]([\w.]+)/.exec(t) || /(webkit)[ \/]([\w.]+)/.exec(t) || /(opera)(?:.*version|)[ \/]([\w.]+)/.exec(t) || /(msie) ([\w.]+)/.exec(t) || /(trident)(?:.*? rv:([\w.]+)|)/.exec(t) || t.indexOf("compatible") < 0 && /(mozilla)(?:.*? rv:([\w.]+)|)/.exec(t) || [];
 n[1] = n[1] || '';
 e[n[1]] = true;
 e.version = n[2];
 if (e.chrome) {
...
function getElText(t) {
 var e = '';
 if (t.textContent !== undefined) {
  e = t.textContent;
 } else if (t.innerText !== undefined) {
  e = t.innerText;
 } else {
  e = jQuery(t).text();
 }
...
BB.build_share_buttons = function() {
 ...
 t['Google +1'] = [ -72, 'https://plus.google.com/share?url={URL}' ];
 var e = $('#soc-container');
...
BB.highlight_cyrillic = function(t) {
 var e = t.html().replace(/[ёа-я]+/gi, '<span class="cyr-symbol">$&</span>');
 t.html(e).addClass('hl-cyrillic');
};
...
BB.FSN = {
 build_nav: function() {
  ...
  t.children('optgroup').each(function() {
   ...
   e += '<li><span class="b">' + t + '</span><ul>';
   $(this).children('option.root_forum').each(function() {
    var t = $(this).html();
    var n = $(this)[0].index;
    e += '<li><span class="f" data-opt_idx="' + n + '">' + t + '</span></li>';
   });
   e += '</ul></li>';
  });
  ...
  $('#fs-nav-ul').html(e).treeview({
   ...
   t[0].selectedIndex = $(this).data('opt_idx');
   $('#fs-nav-list').fadeOut();
  });
 }
};
...
BB.go_to_page = function(t) {
 ...
 window.location = BB.PG_BASE_URL + n + 'start=' + e;
};
...
BB.ET = function() {
 return 'ET' in window && $.isPlainObject(ET) ? ET : null;
};

BB.isHttpsDomain = function() {
 return !!BB.cur_domain.match(/^torrents\.loc|rutracker\.(org|cr|net|nl)$/);
};
...
if (BB.IS_GUEST) {
 $(function() {
  $('#search-text-guest').on('keypress', function(t) {
   if (t.which == 13) {
...
Ajax.prototype = {
 ...
 makeEditable: function(a, t) {
  ...
  if (t == 'input' || t == 'textarea') {
   $('.editable-value', i).val($.trim(r.text()));
  }
...
;// bbcode.js
BB.local_link_reg = new RegExp('^https?://' + location.hostname + '/', 'i');

BB.banned_img_hosts_reg = /centrkino|flashrelease|hidebehind|imagebanana|imageshack|imagevenue|imgur|interfoto|ipicture|photobucket|tinypic|youpic\.ru/i;
...
BB.initSpoilers = function(e) {
 ...
 e.on('click.spoiler', 'div.sp-head', function(e) {
  ...
  if (e.shiftKey) {
   ...
   e.stopPropagation();
   e.shiftKey = false;
   var a = t.hasClass('unfolded');
   $('div.sp-head', $(n.parents('td')[0])).filter(function() {
    return $(this).hasClass('unfolded') ? a : !a;
   }).click();
  } else {
...
BBCode.prototype = {
 ...
 insertAtCursor: function(e) {
  ...
  if (document.selection && document.selection.createRange) {
   ...
   var a = t.value.substr(0, i);
   var s = t.value.substr(r);
   t.value = a + e + s;
 ...
 surround: function(e, t, n) {
  ...
  if (i.setSelectionRange) {
   ...
   var c = i.scrollTop;
   var u = i.value.substr(0, l);
   var f = i.value.substr(o);
   var p = n(i.value.substr(l, o - l));
   var d = e + p + t;
   i.value = u + d + f;
   if (p != '') {
    i.setSelectionRange(l, l + d.length);
    s = true;
   } else {
    i.setSelectionRange(l + e.length, l + e.length);
    s = false;
   }
   i.scrollTop = c;
   if (this.collapseAfterInsert) {
    i.setSelectionRange(l + d.length, l + d.length);
   }
  } else {
   i.value += e + a + t;
  }
 ...
 addTag: function(e, t, n, i, r) {
  ...
  a.ctrlKey = "ctrl";
  a.multiline = r;
  a.elt = this.textarea.form[e];
  this.tags[e] = a;
  var s = a.elt;
  if (s) {
   var l = this;
   if (s.type && s.type.toUpperCase() == "BUTTON") {
    $(s).on('click', function() {
     l.insertTag(e);
     return false;
    });
   }
   if (s.tagName && s.tagName.toUpperCase() == "SELECT") {
...
;// bb.init.js
$(function() {
 $('#adriver-240x120').injectIFrame('//ad.adriver.ru/cgi-bin/erle.cgi', {
  width: '240px',
  height: '120px'
 }, {
  sid: 85725,
 ...
 $(document).ajaxError(function(t, n) {
  ...
  e.html("Ajax error in: <i>" + ajax.url + "</i><br><b>" + a + " " + r + "</b>").show();
 ...
 $('#quick-search').on('submit', function() {
  ...
  case 'wiki':
   var s = 'http://rutracker.wiki/' + encURL('Служебная:Search') + '?fulltext=1&search=' + i;
   post2url(s, {
    target: '_blank'
   });
   return false;
 ...
 BB.getUrlParam = function(t) {
  if (!n) {
   n = a(window.location.search.substring(1));
  }
  return n[t];
 };
 BB.initMagnetLinks = function() {
  $('a.magnet-link').each(function() {
   var t = this;
   var e = $(t).data('topic_id');
   var n = $('.topic-title-' + e).text().substr(0, 150);
   t.href += '&dn=' + encodeURIComponent(n);
  });
 };
 ...
 var r = $('.poster_info:first');
 if (r.length) {
  injectCSS([ '.post_wrap { max-width: ' + Math.round($(window).width() - r.outerWidth() - 32) + 'px }' ].join('\n'));
 }
