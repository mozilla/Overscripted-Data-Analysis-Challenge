   ...
   get: function(elem, opts) {
    ...
    if (settings.type == "html5") {
     ...
     $(elem.attributes).each(function() {
      var t = this.nodeName;
      if (t.match(/^data-/)) t = t.replace(/^data-/, ''); else return true;
      object[t] = getObject(this.nodeValue);
     });
    } else {
...
(function(e) {
 ...
 e.fn.injectBegunAd = function(n) {
  ...
  var i = BB.begun_iframe_src + '?' + e.param(t);
  e(this).injectIFrame(i, t);
 };
 var n = /\B(?=(?:\d{3})+(?!\d))/g;
...
(function(e) {
 ...
 e.modal.prototype = {
  ...
  open: function() {
   this.block();
   this.show();
   var o = this.options.escapeClose;
   e(document).on('keydown.modal', function(t) {
    if (o && t.which == 27) {
     e.modal.close();
    }
    if (e.inArray(t.which, [ 33, 34, 35, 36, 38, 40 ]) != -1) {
     t.preventDefault();
    }
   });
 ...
 e.fn.modal = function(t) {
  if (this.length === 1) {
   o = new e.modal(this, t);
  }
  return this;
 };
 e(document).on('click.modal', 'a[rel="modal:close"]', e.modal.close);
 e(document).on('click.modal', 'a[rel="modal:open"]', function(o) {
  o.preventDefault();
  e(this).modal();
 });
})(jQuery);
...
(function($) {
 $.extend({
  tablesorter: new function() {
   ...
   function buildParserCache(e, t) {
    ...
    if (e.tBodies[0].rows[0]) {
     var s = [], a = i[0].cells, o = a.length;
     for (var n = 0; n < o; n++) {
      var d = false;
      if ($.metadata && ($(t[n]).metadata() && $(t[n]).metadata().sorter)) {
       d = getParserById($(t[n]).metadata().sorter);
      } else if (e.config.headers[n] && e.config.headers[n].sorter) {
       d = getParserById(e.config.headers[n].sorter);
      }
      if (!d) {
       d = detectParserForColumn(e, a[n]);
      }
      if (e.config.debug) {
       r += "column:" + n + " parser:" + d.id + "\n";
      }
      s.push(d);
     }
    }
   ...
   function detectParserForColumn(e, t) {
    var r = parsers.length;
    for (var i = 1; i < r; i++) {
     if (parsers[i].is($.trim(getElementText(e.config, t)), e, t)) {
      return parsers[i];
     }
    }
    return parsers[0];
   ...
   function buildCache(e) {
    ...
    var r = e.tBodies[0] && e.tBodies[0].rows.length || 0, i = e.tBodies[0].rows[0] && e.tBodies[0].rows[0].cells.length || 0, s = e.config.parsers, a = {
     row: [],
     normalized: []
    };
    for (var o = 0; o < r; ++o) {
     var n = e.tBodies[0].rows[o], d = [];
     a.row.push($(n));
     for (var c = 0; c < i; ++c) {
      d.push(s[c].format(getElementText(e.config, n.cells[c]), e, n.cells[c]));
     }
     d.push(o);
     a.normalized.push(d);
     d = null;
    ...
    if (e.config.debug) {
     benchmark("Building cache for " + r + " rows:", t);
     log(a);
    }
    return a;
   }
   function getElementText(e, t) {
    ...
    } else if (typeof e.textExtraction == "function") {
     r = e.textExtraction(t);
    }
    if (r === undefined) {
     r = getElText(t);
    }
   ...
   function appendToTable(e, t) {
    ...
    var i = t, s = i.row, a = i.normalized, o = a.length, n = a[0].length - 1, d = $(e.tBodies[0]), c = [];
    for (var f = 0; f < o; f++) {
     c.push(s[a[f][n]]);
    ...
    if (e.config.appender) {
     e.config.appender(e, c);
    }
    c = null;
    if (e.config.debug) {
     benchmark("Rebuilt table:", r);
    }
    applyWidget(e);
    setTimeout(function() {
     $(e).trigger("sortEnd");
   ...
   function buildHeaders(e) {
    ...
    var r = $.metadata ? true : false, i = [];
    for (var s = 0; s < e.tHead.rows.length; s++) {
     i[s] = 0;
    }
    $tableHeaders = $("thead th", e);
    $tableHeaders.each(function(t) {
     this.count = 0;
     this.column = t;
     this.order = formatSortingOrder(e.config.sortInitialOrder);
     if (checkHeaderMetadata(this) || checkHeaderOptions(e, t)) this.sortDisabled = true;
     if (!this.sortDisabled) {
      $(this).addClass(e.config.cssHeader).find('.tbs-text').after('<span class="tbs-icon">&nbsp;&nbsp;</span>');
     }
   ...
   function checkCellColSpan(e, t, r) {
    var i = [], s = e.tHead.rows, a = s[r].cells;
    for (var o = 0; o < a.length; o++) {
     var n = a[o];
     if (n.colSpan > 1) {
      i = i.concat(checkCellColSpan(e, headerArr, r++));
     } else {
      if (e.tHead.length == 1 || (n.rowSpan > 1 || !s[r + 1])) {
       i.push(n);
      }
     }
    }
   ...
   function setHeadersCss(e, t, r, i) {
   ...
   function updateHeaderSortCount(e, t) {
    var r = e.config, i = t.length;
    for (var s = 0; s < i; s++) {
     var a = t[s], o = r.headerList[a[0]];
     o.count = a[1];
     o.count++;
    }
   }
   ...
   this.construct = function(e) {
    return this.each(function() {
     ...
     var t, r, i, s, a, o = 0, n;
     this.config = {};
     a = $.extend(this.config, $.tablesorter.defaults, e);
     t = $(this);
     i = buildHeaders(this);
     this.config.parsers = buildParserCache(this, i);
     s = buildCache(this);
     var d = [ a.cssDesc, a.cssAsc ];
     fixColumnWidth(this);
     i.on('click', function(e) {
      t.trigger("sortStart");
      var r = t[0].tBodies[0] && t[0].tBodies[0].rows.length || 0;
      if (!this.sortDisabled && r > 0) {
       var o = $(this);
       var n = this.column;
       this.order = this.count++ % 2;
       if (!e[a.sortMultiSortKey]) {
        a.sortList = [];
        if (a.sortForce != null) {
         var c = a.sortForce;
        ...
        a.sortList.push([ n, this.order ]);
       } else {
        if (isValueInArray(n, a.sortList)) {
         for (var f = 0; f < a.sortList.length; f++) {
          var u = a.sortList[f], l = a.headerList[u[0]];
          if (u[0] == n) {
           l.count = u[1];
           l.count++;
           u[1] = l.count % 2;
       ...
       setTimeout(function() {
        setHeadersCss(t[0], i, a.sortList, d);
        appendToTable(t[0], multisort(t[0], a.sortList, s));
       }, 1);
       return false;
      }
     }).on('mousedown', function() {
      if (a.cancelSelection) {
     ...
     t.on("update", function() {
      this.config.parsers = buildParserCache(this, i);
      s = buildCache(this);
     }).on("sorton", function(e, t) {
      $(this).trigger("sortStart");
      a.sortList = t;
      var r = a.sortList;
      updateHeaderSortCount(this, r);
      setHeadersCss(this, i, r, d);
      appendToTable(this, multisort(this, r, s));
     }).on("appendCache", function() {
      appendToTable(this, s);
     }).on("applyWidgetId", function(e, t) {
      getWidgetById(t).format(this);
     }).on("applyWidgets", function() {
      applyWidget(this);
     });
     if ($.metadata && ($(this).metadata() && $(this).metadata().sortlist)) {
      a.sortList = $(this).metadata().sortlist;
     }
     if (a.sortList.length > 0) {
      t.trigger("sorton", [ a.sortList ]);
     }
     applyWidget(this);
    });
   };
   this.addParser = function(e) {
    var t = parsers.length, r = true;
    for (var i = 0; i < t; i++) {
   ...
   this.isDigit = function(e, t) {
    var r = '\\' + t.decimal;
    var i = '/(^[+]?0(' + r + '0+)?$)|(^([-+]?[1-9][0-9]*)$)|(^([-+]?((0?|[1-9][0-9]*)' + r + '(0*[1-9][0-9]*)))$)|(^[-+]?[1-9]+[0-9]*' + r + '0+$)/';
    return RegExp(i).test($.trim(e));
   };
 ...
 ts.addParser({
  ...
  format: function(e, t, r) {
   var i = t.config, s = !i.parserMetadataName ? 'sortValue' : i.parserMetadataName;
   return $(r).metadata()[s];
  },
  type: "numeric"
 });
